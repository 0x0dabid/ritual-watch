...
import { getNftHoldings, getTokenHoldings, getWalletProfile, getWalletTransactions } from "@/lib/data";
...
export default async function AddressPage({ params }: { params: Promise<{ address: string }> }) {
  ...
  const [profile, transactions, tokens, nfts, names, primaryName] = await Promise.all([
  ...
  const txEmptyDescription = databaseEnabled
    ? "This wallet has no indexed transaction history yet. Make sure the indexer has covered this wallet's active block range."
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <TokenHoldingsTable
          ...
          emptyDescription={
            databaseEnabled
              ? "No indexed ERC-20 balances found for this wallet."
      ...
      <section>
        ...
        <NftGrid
          ...
          emptyDescription={
            databaseEnabled
              ? "No indexed NFT balances found for this wallet."
